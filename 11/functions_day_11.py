...
def find_distance(galaxy_a: tuple[int, int], galaxy_b: tuple[int, int], expanding_lines: list[int], expanding_columns: list[int], expansion_factor: int) -> int:
    ...
    distance += sum([line_coords_ordered[0] < line < line_coords_ordered[1] for line in expanding_lines])*(expansion_factor-1)
    distance += sum([column_coords_ordered[0] < column < column_coords_ordered[1] for column in expanding_columns])*(expansion_factor-1)
    return distance


def find_all_distances_between_galaxies(galaxy_coords: tuple[list[int], list[int]], expanding_lines: list[int], expanding_columns: list[int], expansion_factor: int) -> list[int]:
    distances = []
    for galaxy_a_idx in range(0, len(galaxy_coords[0])):
        ...
        for galaxy_b_idx in range(galaxy_a_idx + 1, len(galaxy_coords[0])):
            ...
            distances.append(find_distance(galaxy_a, galaxy_b, expanding_lines, expanding_columns, expansion_factor))
    return distances


def exercise(data: np.ndarray, expansion_factor: int) -> int:
    expanding_lines = find_expanding_lines(data)
    expanding_columns = find_expanding_columns(data)
    galaxy_coords = np.where(data == '#')
    distances = find_all_distances_between_galaxies(galaxy_coords, expanding_lines, expanding_columns, expansion_factor)
