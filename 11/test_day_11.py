...
def test_find_distance():
    assert find_distance((0, 3), (1, 7), [3, 7], [2, 5, 8], 2) == 6
    assert find_distance((0, 3), (2, 0), [3, 7], [2, 5, 8], 2) == 6
    assert find_distance((1, 7), (2, 0), [3, 7], [2, 5, 8], 2) == 10


def test_find_all_distances_between_galaxies():
    assert find_all_distances_between_galaxies(([0, 1, 2], [3, 7, 0]), [], [2, 5, 8], 2) == [6, 6, 10]


def test_exercise_1():
    assert exercise(data_small, 2) == 374


def test_exercise_1_all_data():
    assert exercise(data_full, 2) == 9565386


def test_exercise_2_all_data():
    assert exercise(data_full, 1000000) == 857986849428
